...
Console.WriteLine("Ordem natural: ");
...
int estadoCivilDigitado = 0;
while(!Int32.TryParse(Console.ReadLine(), out estadoCivilDigitado)
    || estadoCivilDigitado < 1 || estadoCivilDigitado > 3)
{
    Console.WriteLine("Informe um estado civil entre 1 e 3");
}

string estadoCivil = estadosCivis[estadoCivilDigitado - 1];

Console.WriteLine($"Selecionado: {estadoCivil}");
...
string[] sobrenomes = ["Santos", "Gonçalves", "Carvalho", "Moura"];
...
for (int i = 1; i <= quantidade; i++)
{
    ...
    string nomeCompleto = $"{nomes[sorteioNome]} {sobrenomes[sorteioSobrenome]}";
    Console.WriteLine($"{i}º nome sorteado: {nomeCompleto}");
}
